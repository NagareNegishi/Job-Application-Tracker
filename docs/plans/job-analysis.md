...
- **Profile quality score (separate plan):** full weighted 0–100 meter + per-field improvement hints, client-side only (like the dashboard). A lightweight "meets minimum but thin" advisory can ship earlier.
...
- **Profile save error messages (known issue):** generic "Failed to save." doesn't surface API detail; distinguish network vs validation errors when polishing.
