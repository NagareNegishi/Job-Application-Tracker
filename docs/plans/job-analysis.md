...
| 4 | Add `/profile` page to frontend — tags input for `TargetRoles` and `Skills`; repeating entry forms for `WorkHistory` and `Education` (add/remove entries) | In progress — route + nav done (Step 5 folded in); tag sections (E) done + `TagInput` polished (see Notes); **next: tag-input autocomplete (sources decided — see Notes), then** working rights (F), work history (G), education (H) |
...
- **Tag-input autocomplete (Step 4 continuation, do before Working Rights F):** each tag section suggests completions as the user types (with scrollable dropdown).
  ...
  - **Full list generation (deferred — separate session):** `components/profile/tagSuggestions.ts` currently holds PLACEHOLDER seeds for Skills / Certifications / Target Roles. Generate the real curated lists in a dedicated session — no repo context needed, just replace the const arrays.
