...
- **Profile quality score (in progress 2026-07-10):** scoring util done — `utils/profileScore.ts`, config-driven (`PROFILE_SCORE_CONFIG`: per-section weights + grading strategy), normalizes by total weight, returns `{ score, breakdown }`. Unit tests in `profileScore.test.ts`. `ProfilePage` renders the raw score as temporary plain text. Grading: WorkHistory 25 (quality+count — description ≥40 chars = "complete"), Skills 25 (3-tier count: 1–2/3–4/5+), TargetRoles 15, Education/WorkingRights/Languages 10 (presence), Certs 5 (lenient presence). **Next session:** build a display component (meter or labelled band) to replace the plain text; then per-field improvement hints — the returned `breakdown` already carries per-section `fraction`/`points` to target weak sections.
