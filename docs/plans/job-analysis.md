...
- WorkHistory/Education entry (frontend): per-entry "Currently working/enrolled here" checkbox — when checked, To row is hidden and `toYear: null` sent. When unchecked, To row appears; save blocked until a year is selected (`toYear === 0` sentinel). WorkHistory: title + company required (`*` indicator); save blocked if either empty. "Add role/education" button disabled while section is dirty. From/To use separate Month (optional) + Year (required) Select dropdowns; `fromYear === 0` / `toYear === 0` are the "not yet selected" sentinels that block save. Known polish: select dropdowns show a brief render lag on first interaction — deferred.
...
- **EducationSection (done 2026-07-10):** `EducationSection.tsx` built — institution + degree (`SuggestionInput` with placeholder lists), "currently enrolled" checkbox, start/end `YearSelect`; `INSTITUTION_SUGGESTIONS` (placeholder) + `DEGREE_SUGGESTIONS` (curated) added to `tagSuggestions.ts`; wired into `ProfilePage`. **Polish done 2026-07-10:** start/end year in `flex gap-2` row, always 50/50 width (empty right column when current so start year never stretches full-width); label → "School or institution"; degree placeholder → "e.g. BSc Computer Science".
- **`checkDateOrder` util (done 2026-07-10):** `src/utils/dateValidation.ts` — validates from→to date ordering; 3 TS overloads (year-only, year+month, year+month+day); caller chooses granularity; month/day ordering only checked when both values present. Wired into `WorkHistorySection` and `EducationSection`: per-entry inline error after the date row, Save blocked when any entry has an ordering violation. Replaces the misleading "Failed to save" that appeared when users entered impossible date combinations.
