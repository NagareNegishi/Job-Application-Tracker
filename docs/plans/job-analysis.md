...
| 4 | Add `/profile` page to frontend — tags input for `TargetRoles` and `Skills`; repeating entry forms for `WorkHistory` and `Education` (add/remove entries) | In progress — route + nav done (Step 5 folded in); tag sections (E) done + `TagInput` polished + tag-input autocomplete done (placeholder lists — see Notes); **next: working rights (F), work history (G), education (H)** |
