...
- **EducationSection (done 2026-07-10):** `EducationSection.tsx` built — institution + degree (`SuggestionInput` with placeholder lists), "currently enrolled" checkbox, start/end `YearSelect`; `INSTITUTION_SUGGESTIONS` (placeholder) + `DEGREE_SUGGESTIONS` (curated) added to `tagSuggestions.ts`; wired into `ProfilePage`. **Polish deferred to next session** (layout, copy, UX refinements — same pass as WorkHistory polish) before sub-step I.
