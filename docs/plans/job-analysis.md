...
| 4 | Add `/profile` page to frontend — tags input for `TargetRoles` and `Skills`; repeating entry forms for `WorkHistory` and `Education` (add/remove entries) | In progress — route + nav done (Step 5 folded in); tag sections (E) done + `TagInput` polished + tag-input autocomplete done (placeholder lists — see Notes); working rights (F) built: `countryCodes.ts` + `CountryCombobox.tsx` + `WorkingRightsSection.tsx` done and wired into `ProfilePage.tsx`; **next: polish F, then work history (G), education (H), suggestion lists (I — last, separate session)** |
