...
- **New — a per-job document cap already exists, but at the wrong value.** `DocumentsController.cs:108-111` already blocks uploads past `ValidationConstants.MaxDocumentPerJob`, currently a flat **10** applied to every user (`ValidationConstants.cs:20`), not the product doc's Free-tier value of 1. Step 2 is "make an existing global constant tier-aware," not "add a new check" — and it raises a question the product doc doesn't answer: does Paid keep something like today's 10-per-job ceiling, or go truly unbounded? Worth a quick decision before Step 2 starts.
- **New — the controller exposing the 5-call job-analysis feature wasn't found.** `ClaudeAnalysisService`'s five methods (`AnalyseAlignmentAsync`, etc.) are registered in `Program.cs:159` and rate-limited via a 5/min `"analyse"` policy (`Program.cs:253-259`) — but no controller calling them turned up in this pass, and CLAUDE.md's own controller list doesn't mention one either. Step 3's instrumentation touches whatever calls these methods; find that controller before starting.
