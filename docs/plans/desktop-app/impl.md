...
Add `tauri-plugin-keyring` (v0.2.0, actively maintained, covers macOS Keychain / Windows Credential Manager / Linux Secret Service, with a `linux-keyutils` fallback) and a settings screen where the user enters their Claude API key, stored in the keychain, never a config file or localStorage. The backend reads the key from the keychain at call time rather than from `appsettings`. Reuse the existing `SettingsPage` (confirmed present) as the host.
