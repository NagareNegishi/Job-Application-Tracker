...
| Profile inline view/edit | All 12 profile sections show a read-only view with pencil → in-place edit; shared `ProfileSectionCard`; empty sections show placeholder text + header + button that seeds a blank entry (multi-entry sections also get add-another with scroll); Edit all / Save all / Cancel all in header; auto edit-all when no profile exists; validation lifted to `utils/profileValidation.ts`; see `docs/plans/profile-inline-edit.md` | **Active priority — comes before Job analysis.** Committed. Section-wiring refactor batch 1 + entry-list extraction (`useEntryList` + `EntryRow`) + salary → per-currency array + Languages model (combined with fluency) all done and committed 2026-07-17/18. Next (the profile page + model sort-out that gates Job analysis): drafts state model, scoring rules for the 5 newer profile fields (`profileScore` exhaustive type constraint relaxed until then), and the remaining simplifications / known follow-ups — full list in `docs/plans/profile-inline-edit.md`. Salary array (`salaryExpectations`, max 3, distinct currencies) fixes the PATCH null-clearing bug (`[]` clears where `null` couldn't), adds multi-currency, moves salary onto `useEntryList`; migration `SalaryExpectationsArray`. Languages (`languages: LanguageEntry[]`, `{ language, fluency }`) mirrors `WorkingRightEntry`; 5-level LinkedIn-style `LanguageFluency` enum + `Unspecified` for migrated rows; migration `LanguagesFluencyEntry` (`text[]` → `jsonb`, needed a rename/backfill/drop workaround since Postgres rejects a correlated subquery in `ALTER COLUMN ... USING`). |
