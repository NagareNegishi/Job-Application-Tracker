...
| Profile inline view/edit | All 12 sections show a read-only view with pencil → in-place edit; shared `ProfileSectionCard`; empty sections seed a blank entry via a + header button; Edit all / Save all / Cancel all; auto edit-all when no profile exists; validation in `utils/profileValidation.ts`; see `docs/plans/profile-inline-edit.md` | **Active priority — before Job analysis.** Core refactor + model changes (salary → per-currency array, Languages + fluency, duplicate protection) committed 07-17/18. Scoring for the 5 newer fields done (`profileScore.ts` weights preference fields; total weight 100→108). Language free-text validation dropped (English-first app, self-inflicted-only risk, existing 30-char cap sufficient). Remaining before Job analysis: small `tagSuggestions.ts` dynamic-import cleanup. Drafts state-model refactor reassessed as a simplification (no live bug) and deferred. Detail + commit history in `docs/plans/profile-inline-edit.md`. |
