...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Profile (Steps 1–5) done. Step 6 done: `IAnalysisService`, `ClaudeAnalysisService`, `ClaudeAnalysisConfig`, `AnalysisModels`. Profile Conditions expansion: C1–C5 done (new fields/entity/migration/DTO wiring + `ProfileDTOTests` coverage). Next: C6 (`AlignmentPrompt` rework + `concern` field), then Step 6 polish (prompt quality + `ExtractJson`/`LogContractIssues` shared helper refactor), then Steps 7–10 / C7–C8. See `docs/plans/job-analysis.md`. |
