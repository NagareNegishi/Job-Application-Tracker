...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Steps 1–3 + 3a/3b (tests) done. Step 4 in progress: route + nav done; tag sections + `TagInput` polish + autocomplete done; working rights (F) built + polished (fluid combobox, responsive row, per-entry border card, Trash2 icon, duplicate-country prevention via `excludeCodes`, Save blocked on empty country, copy polish); **next: work history (G), education (H)**. See `docs/plans/job-analysis.md`. |
