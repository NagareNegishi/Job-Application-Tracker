...
| Profile inline view/edit | All 12 sections show a read-only view with pencil → in-place edit; shared `ProfileSectionCard`; empty sections seed a blank entry via a + header button; Edit all / Save all / Cancel all; auto edit-all when no profile exists; validation in `utils/profileValidation.ts`; see `docs/plans/profile-inline-edit.md` | **Active priority — before Job analysis.** Committed (2026-07-17/18): section-wiring refactor, entry-list extraction (`useEntryList` + `EntryRow`), salary → per-currency array (`salaryExpectations`, max 3, distinct currencies — `[]` clears where PATCH `null` couldn't; migration `SalaryExpectationsArray`), Languages + fluency (`LanguageEntry`, 5-level `LanguageFluency`; migration `LanguagesFluencyEntry`), duplicate protection. Next (gates Job analysis): drafts state model, scoring for the 5 newer fields, creatable language combobox, remaining follow-ups — see plan doc. |
