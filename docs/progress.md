...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Profile (Steps 1–5) + Conditions expansion (C1–C7) done; C7 components still need a layout/visual polish pass (not blocking). Step 6 + 6a done: `IAnalysisService`, `ClaudeAnalysisService`, shared `ClaudeResponseHelper`. Steps 7, 8, 8a done: `AnalysisController` at `/api/analyse` (5 endpoints, demo-block, profile gate, `AiEnabled` policy, `"analyse"` 5/min rate limit), `ClaudeAnalysisService` registered in `Program.cs`, `AnalysisControllerTests` (13 tests, mocked `IAnalysisService`). All 5 prompts in `ClaudeAnalysisConfig.cs` have had a wording/structure polish pass. Remaining: 8b (live-test prompts against real listings and iterate), Steps 9–10 (analysis UI + ad-hoc entry point), C8 (frontend `concern` display). See `docs/plans/job-analysis.md`. |
...
| Profile inline view/edit | All 12 sections show a read-only view with pencil → in-place edit; shared `ProfileSectionCard`; empty sections seed a blank entry via a + header button; Edit all / Save all / Cancel all; auto edit-all when no profile exists; validation in `utils/profileValidation.ts`; see `docs/plans/profile-inline-edit.md` | Done — mandatory work complete (core refactor + model changes committed 07-17/18; scoring for the 5 newer fields done; route-level lazy loading resolved the `tagSuggestions.ts` bundle-split concern). Remaining items are optional polish (drafts state-model simplification, minor UX follow-ups), not blocking. Job analysis is now the active priority. Detail + commit history in `docs/plans/profile-inline-edit.md`. |
