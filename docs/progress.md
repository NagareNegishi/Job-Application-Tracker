...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Steps 1–2 done. Next: **Step 3** — add `GET /api/account/profile`, `PUT /api/account/profile`, `PATCH /api/account/profile` to `AccountController`. GET returns `{}` (not 404) when no profile exists; PUT returns 409 if profile already exists; PATCH applies merge-patch (404 if no profile yet). See `docs/plans/job-analysis.md`. |
