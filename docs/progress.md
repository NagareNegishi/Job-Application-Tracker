...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Steps 1–3 + 3a/3b (tests) done. Step 4 in progress: route + nav done; tag sections + `TagInput` polish + autocomplete done; working rights (F) done + polished; work history (G) done + polished (`MonthYearPicker` + `MonthSelect` + `YearSelect` extracted; `SuggestionInput` for title autocomplete reusing `TARGET_ROLE_SUGGESTIONS`; `select.tsx` global `w-full` + `popper` fix; `matchSuggestion` multi-word `word-start` fix + 2 new tests); education (H) done + built; **next: polish EducationSection, then suggestion lists (I)**. See `docs/plans/job-analysis.md`. |
