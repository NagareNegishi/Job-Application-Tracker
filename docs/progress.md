...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Steps 1–3 + 3a/3b (tests) done. Step 4 in progress: route + nav done; tag sections done + `TagInput` polished; **next: tag-input autocomplete (next session), then working rights (F), work history (G), education (H)**. See `docs/plans/job-analysis.md`. |
