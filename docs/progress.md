...
| Profile inline view/edit | All 12 profile sections show a read-only view with pencil → in-place edit; shared `ProfileSectionCard`; empty sections show placeholder text + header + button that seeds a blank entry (multi-entry sections also get add-another with scroll); Edit all / Save all / Cancel all in header; auto edit-all when no profile exists; validation lifted to `utils/profileValidation.ts`; see `docs/plans/profile-inline-edit.md` | Committed. Section-wiring refactor batch 1 done 2026-07-17 (shared `SectionProps<T>` + `sectionProps(key)` spread + page-computed dirty; −146 lines), **uncommitted**. Next: entry-list extraction (`useEntryList` + `EntryRow`), then drafts state model — list in plan doc. `profileScore` exhaustive type constraint relaxed until new fields get scoring rules. Salary → per-currency array (`salaryExpectations`, max 3, distinct currencies) 2026-07-17: fixes the PATCH null-clearing bug (`[]` clears where `null` couldn't), adds multi-currency, moves salary onto `useEntryList`; migration `SalaryExpectationsArray`; **uncommitted**, pending browser check |
