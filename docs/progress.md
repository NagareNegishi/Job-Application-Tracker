...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Profile (Steps 1–5) + Conditions expansion (C1–C7) done; C7 components still need a layout/visual polish pass (not blocking). Backend done: `IAnalysisService`/`ClaudeAnalysisService`, `AnalysisController` at `/api/analyse` (5 endpoints, demo-block, profile gate, `AiEnabled` policy, `"analyse"` 5/min rate limit, prompts polished), `AnalysisControllerTests` (13 tests). Remaining: Steps 9–10 (analysis UI + ad-hoc entry point), C8 (frontend `concern` display). See `docs/plans/job-analysis.md`. |
