...
| Job analysis | `UserProfile` table + 5 AI analysis endpoints + detail page UI | Steps 1–3 + 3a/3b (tests) done. Step 4 in progress: route + nav done; tag sections + `TagInput` polish + autocomplete done; `matchesSuggestion` util + 10 Vitest tests added (`src/utils/`); `TagInput` `matchStrategy` prop wired (default `"word-start"`, Languages uses `"prefix"`); **next: polish working rights (F), then work history (G), education (H)**. See `docs/plans/job-analysis.md`. |
