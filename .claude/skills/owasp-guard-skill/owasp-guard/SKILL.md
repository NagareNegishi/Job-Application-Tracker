...
Cache location: `${CLAUDE_SKILL_DIR}/cache/`
...
Pre-cached cheat sheets are bundled with the skill in `${CLAUDE_SKILL_DIR}/cache/`.
To update the cache, the user runs `/owasp-update` (see the `owasp-update` skill).
This skill never fetches cheat sheets itself.

### Staleness Check

On first trigger per session, read `${CLAUDE_SKILL_DIR}/cache/last_updated.json`.
If any required cheat sheet has `fetched_at` older than 90 days, suggest once:
...
Detect language/framework. Read `${CLAUDE_SKILL_DIR}/references/<language>.md` to determine
which OWASP categories and cheat sheets are relevant.

Available: `${CLAUDE_SKILL_DIR}/references/python.md`, `${CLAUDE_SKILL_DIR}/references/javascript.md`,
`${CLAUDE_SKILL_DIR}/references/go.md`, `${CLAUDE_SKILL_DIR}/references/java.md`,
`${CLAUDE_SKILL_DIR}/references/csharp.md`
...
`${CLAUDE_SKILL_DIR}/cache/` for guidance. If a required cheat sheet is not cached,
